...
const InternProfile = () => {
  ...
  const filteredSubmissions = mockSubmissions
    ...
    .filter((submission) =>
      submission.contents[0].value
        .toLowerCase()
        .includes(searchTerm.toLowerCase())
    )
    .filter((submission) =>
      filterScore ? String(submission.grade) === filterScore : true
    );

  const averageScore =
    filteredSubmissions.reduce(
      (acc: number, submission: Submission) =>
        acc + (Number(submission.grade) || 0),
      0
